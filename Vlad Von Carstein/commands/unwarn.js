...
module.exports = {
    run: async (message, args, client) => {
        ...
        if (message.author === member.user) return message.channel.send('*Tu ne peux pas enlever tes propres warns.*')
        if (!client.db.warns[member.id]) return message.channel.send('Ce membre n\'a aucun warn.')
        const warnIndex = parseInt(args[1], 10) - 1
        if (warnIndex < 0 || !client.db.warns[member.id][warnIndex]) return message.channel.send('Indiquez un warn.')
    ...
    name: 'unwarn',
    help: {
        description: '**Description :** Cette commande permet de retirer un avertissement à un membre.\n\n**Rôle requis :** @Modérateurs',
        syntax: "<@membre> [numéro du warn à enlevé]"
    },
    guildOnly: true
}